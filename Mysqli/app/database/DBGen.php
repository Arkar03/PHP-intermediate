...
class DBGen
{
    ...
    public function insertSingleShop($name, $ipadd, $username, $pass, $state)
    {
        $date = date("Y-m-d H:m:s");
        $stmt = $this->conn->prepare("INSERT INTO shops(name,ipadd,username,password,state,created_at) VALUES(?,?,?,?,?,?)");

        $stmt->bind_param('ssssis', $name, $ipadd, $username, $pass, $state, $date);
    ...
    public function insertMultipleShop($shops)
    {
        ...
        foreach ($shops as $shop) {
            ...
            $result = $stmt->execute();
            $lasetinsertid = $stmt->insert_id;
            echo "Successfully insert " . $shop[0] . " with id of " . $lasetinsertid . "<br>";
        }
    }
